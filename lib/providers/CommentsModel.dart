...
class CommentsModel with ChangeNotifier {
  ...
  CommentsModel(BuildContext context, String? feedType, int? postId,
  ...
  Future<void> fetchComments() async {
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadcomments),
          body: jsonEncode({
            "data": {"feed_type": feedType, "id": 0, "post_id": postId}
          }));
  ...
  Future<void> deleteComment(int? commentId, int position) async {
    ...
    try {
      var data = {"id": commentId, "feed_type": feedType, "post_id": postId};
      print(data.toString());
